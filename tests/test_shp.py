...
GEOGRAPHY_CB = Literal["county_sub", "tracts"]
...
@pytest.mark.parametrize("abbr,year,geom", [(abbr, y, geom) for y in range(2014, 2022) for abbr in ["AL", "CA", "MD"] for geom in get_args(GEOGRAPHY_CB)])
def test_states(abbr: str, year: int, geom: GEOGRAPHY):
    st = State.from_abbr(abbr=abbr)
    _test_cb(unit=st, geom=geom, year=year)


@pytest.mark.parametrize("st_abbr,name,year,geom", [("MD", "Montgomery", y, geom) for y in range(2014, 2022) for geom in get_args(GEOGRAPHY_CB)])
